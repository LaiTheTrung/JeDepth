...
class BridgeNode(Node):
    def __init__(self):
        ...
        # Republish static TF periodically to ensure RViz gets it even if it restarts
        ...
        self.depth_sub = self.create_subscription(
            Image, '/stereo/depth_map', self.depth_callback, 10  #cam0/depthmap
        )
        ...
        # Local Map Visualization (Bounding Box)
        # Size must match 'grid_map/local_update_range' in advanced_param.launch.py
        # Range x=7.5 -> Size=15.0, y=7.5 -> Size=15.0, z=4.0 -> Size=8.0
        self.local_map_pub = self.create_publisher(Marker, f'/drone_{self.drone_id}_ego_planner_node/local_map_bound', 10)
        self.local_map_size = [15.0, 15.0, 8.0]
    ...
    def publish_static_tf(self):
        ...
        """
        t = TransformStamped()
        # Use 0 timestamp for static transforms to be valid for all time
    ...
    def mission_loop(self):
        ...
        # Visualize Local Map (10Hz)
        if self.latest_odom_enu is not None:
            self.publish_local_map_marker(now, self.latest_odom_enu['pos'])
        ...
        if self.state == "INIT":
            if elapsed > 1.0: # Wait a bit for data to stabilize
                ...
                if altitude < 0.2: # On Ground -> Standard Takeoff
    ...
    def publish_local_map_marker(self, timestamp, pos_enu):
        """
        Publish a wireframe/transparent box representing the Local Map Update Range.
        This helps visualize where the planner is actively updating the map.
        """
        marker = Marker()
