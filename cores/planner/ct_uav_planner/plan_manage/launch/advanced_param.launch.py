    ...
    ego_planner_node = Node(
        ...
        parameters=[
            # local_planner_para_dir,#9
                    ...
                    {'fsm/thresh_replan_time': 0.5},          # [REAL] Tăng lên 0.5s (sim: 0.35s) — ré plan ít hơn, mượt hơn
                    {'fsm/thresh_no_replan_meter': 0.3},       # [REAL] Giảm xuống 0.3m (sim: 0.6m) — bay vào gần đích hơn mới dừng
                    ...
                    {'grid_map/local_update_range_z': 4.0},
                    {'grid_map/obstacles_inflation': 0.05},   # Margin va chạm 30cm cho thân máy bay
                    {'grid_map/local_map_margin': 10},
                    {'grid_map/ground_height': -2.0},  # Allow 1m below starting position (for slopes)
                    {'grid_map/odom_depth_timeout': 3.0}, # Increased from 1.0 to prevent false positives
                    ...
                    {'grid_map/depth_filter_mindist': 0.6},   # Tăng từ 0.45 → 0.6 để loại bỏ noise cực gần
                    ...
                    {'grid_map/p_max': 0.95},
                    {'grid_map/p_occ': 0.60},
                    {'grid_map/min_ray_length': 0.45},
                    {'grid_map/max_ray_length': 8.0},

                    {'grid_map/virtual_ceil_height': 10.0},
                    {'grid_map/visualization_truncate_height': 10.0},
                    {'grid_map/show_occ_time': False},
                    {'grid_map/pose_type': 1},  # POSE_STAMPED -> Perfectly matched timestamp with Depth from bridge_real_ver2.py
                    ...
                    {'optimization/lambda_smooth': 1.2},
                    {'optimization/lambda_collision': 2.0}, # Tăng gấp đôi độ ưu tiên né va chạm
                    {'optimization/lambda_feasibility': 0.1},
                    {'optimization/lambda_fitness': 1.0},
                    {'optimization/dist0': 0.5},              # Giảm từ 0.75 → 0.5 để planner không quá conservative
