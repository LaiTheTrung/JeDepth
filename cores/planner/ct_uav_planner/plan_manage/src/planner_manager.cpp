...
namespace ego_planner
{
  ...
  {
    // BUG FIX: In ROS2, wall clock time is ~1.7e9s, so `< 1e9` was ALWAYS true → always
    // returned false (skipping swarm check entirely). Use traj_id_ == 0 instead which
    // correctly means "no trajectory has been planned yet".
    if (local_data_.traj_id_ == 0) // My first planning has not started
      return false;
