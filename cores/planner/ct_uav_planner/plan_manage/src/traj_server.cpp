...
rclcpp::Clock _clock;
bool odom_yaw_initialized_ = false;  // [FIX] Track whether we have real yaw from odom

void bsplineCallback(const std::shared_ptr<traj_utils::msg::Bspline> msg)
{
  // parse pos traj
  if (msg->pos_pts.empty()) {
    RCLCPP_WARN(rclcpp::get_logger("traj_server"), "Received empty B-spline!");
    return;
  }
...
// [FIX] Odometry callback: keep last_yaw_ in sync with real drone yaw
// until the first trajectory is received. This prevents the initial yaw jerk.
void odomCallback(const std::shared_ptr<nav_msgs::msg::Odometry> msg)
{
  if (receive_traj_) return;  // Once trajectory running, traj_server owns last_yaw_

  // Extract yaw from quaternion (NED → ENU: qz component gives yaw in world frame)
...
std::pair<double, double> calculate_yaw(double t_cur, Eigen::Vector3d &pos, rclcpp::Time &time_now, rclcpp::Time &time_last)
{
  ...
  constexpr double YAW_DOT_MAX_PER_SEC = 1.0; // Reduced from PI to prevent oscillation
...
int main(int argc, char **argv)
{
  ...
  // [FIX] Subscribe to odom to track real yaw BEFORE first trajectory
  std::string odom_topic = "/drone_0_visual_slam/odom";
  nh->declare_parameter<std::string>("odom_topic", odom_topic);
  nh->get_parameter("odom_topic", odom_topic);
  auto odom_sub = nh->create_subscription<nav_msgs::msg::Odometry>(
      odom_topic, 10, odomCallback);
  RCLCPP_INFO(rclcpp::get_logger("traj_server"), "[FIX] Odom yaw init from: %s", odom_topic.c_str());
  ...
  RCLCPP_INFO(rclcpp::get_logger("traj_server"), "[FIX] Subscribing to odom to initialize yaw...");
