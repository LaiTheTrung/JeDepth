...
void odom_callback(const nav_msgs::msg::Odometry::SharedPtr msg)
{
  ...
  static double TEM_A=prevt.seconds();
  static double TEM_B=prevt.seconds();
  rclcpp::Time tem_c=msg->header.stamp;//这里TEM_A是被动的prevt值      TEM_B是主动的值
  TEM_B = tem_c.seconds();
  if ((TEM_B - TEM_A) > 0.1)
  {
    TEM_A = TEM_B;
  ...
  if ((t - pt).seconds() > 0.5)
  {
    ...
    trajROS.header.stamp    = _clock.now();
...
int main(int argc, char** argv)
{
  ...
  auto n = std::make_shared<rclcpp::Node>("odom_visualization");//这个只是说节点的名字叫这个

  n->declare_parameter<std::string>("mesh_resource",std::string("package://odom_visualization/meshes/hummingbird.mesh"));
  ...
  n->declare_parameter<int>("drone_id",-1);
  ...
  n->get_parameter("drone_id",_drone_id);
  ...
  rclcpp::Subscription<nav_msgs::msg::Odometry>::SharedPtr sub_odom 
    = n->create_subscription<nav_msgs::msg::Odometry>("/odom",100,odom_callback);
  ...
  rclcpp::Subscription<quadrotor_msgs::msg::PositionCommand>::SharedPtr sub_cmd 
    = n->create_subscription<quadrotor_msgs::msg::PositionCommand>("/cmd",100,cmd_callback);

  // posePub   = n.advertise<geometry_msgs::msg::PoseStamped>("pose",                100, true);
  rclcpp::QoS qos(rclcpp::KeepLast(100)); 
  posePub = n->create_publisher<geometry_msgs::msg::PoseStamped>("/pose", qos);
  // pathPub   = n.advertise<nav_msgs::Path>(            "path",                100, true);
  pathPub = n->create_publisher<nav_msgs::msg::Path>("/path", qos);
  // velPub    = n.advertise<visualization_msgs::msg::Marker>("velocity",            100, true);
  velPub  = n->create_publisher<visualization_msgs::msg::Marker>("/velocity", qos);
  // covPub    = n.advertise<visualization_msgs::msg::Marker>("covariance",          100, true);
  covPub  = n->create_publisher<visualization_msgs::msg::Marker>("/covariance", qos);
  // covVelPub = n.advertise<visualization_msgs::msg::Marker>("covariance_velocity", 100, true);
  covVelPub  = n->create_publisher<visualization_msgs::msg::Marker>("/covariance_velocity", qos);
  // trajPub   = n.advertise<visualization_msgs::msg::Marker>("trajectory",          100, true);
  trajPub  = n->create_publisher<visualization_msgs::msg::Marker>("/trajectory", qos);
  // sensorPub = n.advertise<visualization_msgs::msg::Marker>("sensor",              100, true);
  sensorPub  = n->create_publisher<visualization_msgs::msg::Marker>("/sensor", qos);
  // meshPub   = n.advertise<visualization_msgs::msg::Marker>("robot",               100, true);  
  meshPub  = n->create_publisher<visualization_msgs::msg::Marker>("/robot", qos);
  // heightPub = n.advertise<sensor_msgs::Range>(        "height",              100, true);  
  heightPub  = n->create_publisher<sensor_msgs::msg::Range>("/height", qos);
