    ...
    while (!stop_flag_ && rclcpp::ok()) {
        ...
        try {
            ...
            if (response.image_data_uint8.size() == response.width * response.height * 3) {
                ...
                cv::cvtColor(rgb.clone(), img, cv::COLOR_RGB2GRAY);
                ...
                cv::Mat decoded = cv::imdecode(data, cv::IMREAD_GRAYSCALE);
                if (decoded.empty()) {
                    RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 1000,
...
RCLCPP_COMPONENTS_REGISTER_NODE(ct_uav_omni::FastStreamNode)