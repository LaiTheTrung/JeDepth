#include "fast_stream_node_cassini.hpp"
...
FastStreamNodeCassini::FastStreamNodeCassini(const rclcpp::NodeOptions & options)
      ...
      coordinator_fps_counter_(30)
{
    ...
    this->declare_parameter<std::string>("cassini_map_file", "/mnt/d/trung_Nav_team/Cosys-AirSim/ros2/src/ct_uav_omni_fast_stream/config/cassini_maps.yaml");
    ...
    this->get_parameter("cassini_map_file", cassini_map_file_);
...
FastStreamNodeCassini::~FastStreamNodeCassini()
{
    stopThreads();
    RCLCPP_INFO(this->get_logger(), "Fast Stream Node stopped");
}

void FastStreamNodeCassini::initializeAirSimClients()
{
...
void FastStreamNodeCassini::startThreads()
{
    stop_flag_ = false;
    
    // Start capture coordinator thread (steps 1-4)
    capture_thread_ = std::thread(&FastStreamNodeCassini::captureCoordinatorThreadFunc, this);

    // Start publish thread (step 5)
    publish_thread_ = std::thread(&FastStreamNodeCassini::publishThreadFunc, this);
    
    // Start camera threads
    for (size_t i = 0; i < NUM_CAMERAS; ++i) {
        camera_threads_[i] = std::thread(&FastStreamNodeCassini::cameraThreadFunc, this, i);
    }
}

void FastStreamNodeCassini::stopThreads()
{
    stop_flag_ = true;
...
void FastStreamNodeCassini::captureCoordinatorThreadFunc()
{
    RCLCPP_INFO(this->get_logger(), "[CaptureCoordinator] Started");

    if (PERFECT_SYNC) {
        RCLCPP_INFO(this->get_logger(), "[CaptureCoordinator] PERFECT SYNC: PAUSE 0 CAPTURE 0 UNPAUSE");
    }
    simulation_fps_counter_.tick();
    while (!stop_flag_ && rclcpp::ok()) {
        // Track FPS only for capture loop
        coordinator_fps_counter_.tick();

        // ===== STEP 1: PAUSE simulation =====
        if (PERFECT_SYNC && sim_control_client_) {
            try {
                sim_control_client_->simPause(true);
                auto [inst_fps, avg_fps] = simulation_fps_counter_.tick();
                RCLCPP_INFO(this->get_logger(),
                    "[CaptureCoordinator] Simulation Loop FPS: %.1f (avg: %.1f)", inst_fps, avg_fps);
            } catch (const std::exception& e) {
                RCLCPP_WARN(this->get_logger(), "[CaptureCoordinator] Pause error: %s", e.what());
            }
        }

        // Decide which buffer we will write this cycle
        int current_write = write_index_.load(std::memory_order_relaxed);
        FrameArray& target_buffer = frame_buffers_[current_write];
            ...
            while (cameras_done_counter_ < NUM_CAMERAS && !stop_flag_) {
                if (capture_cv_.wait_until(lock, timeout) == std::cv_status::timeout) {
                    RCLCPP_WARN(this->get_logger(),
                        "[CaptureCoordinator] Timeout: only %zu/%zu cameras completed",
        ...
        if (PERFECT_SYNC && sim_control_client_) {
            try {
                ...
                RCLCPP_WARN(this->get_logger(), "[CaptureCoordinator] Unpause error: %s", e.what());
            }
        }

        // Copy latest per-camera frames into target_buffer (no locking between threads)
        for (size_t i = 0; i < NUM_CAMERAS; ++i) {
            std::lock_guard<std::mutex> lock(frame_mutexes_[i]);
            target_buffer[i] = latest_frames_[i];
        }

        // Mark this buffer as ready for publishing and switch to the other buffer
        {
            std::lock_guard<std::mutex> lock(publish_mutex_);
            ready_index_.store(current_write, std::memory_order_release);
        }
        publish_cv_.notify_one();

        // Switch write index for next iteration (ping-pong between 0 and 1)
        write_index_.store(1 - current_write, std::memory_order_relaxed);
    }

    RCLCPP_INFO(this->get_logger(), "[CaptureCoordinator] Stopped");
}

void FastStreamNodeCassini::publishThreadFunc()
{
    RCLCPP_INFO(this->get_logger(), "[Publish] Started");

    uint64_t loop_count = 0;
    publish_fps_counter_.tick();
    while (!stop_flag_ && rclcpp::ok()) {
        int buffer_index = -1;
        ...
        // Wait for a buffer to become ready
        {
            ...
            publish_cv_.wait(lock, [&]() {
                return stop_flag_.load() || ready_index_.load(std::memory_order_acquire) != -1;
            });
            ...
            buffer_index = ready_index_.load(std::memory_order_acquire);
            ready_index_.store(-1, std::memory_order_release);
        }
        ...
        FrameArray& frames_snapshot = frame_buffers_[buffer_index];

        bool all_valid = true;
        for (size_t i = 0; i < NUM_CAMERAS; ++i) {
            if (!frames_snapshot[i].valid || frames_snapshot[i].image.empty()) {
                all_valid = false;
                RCLCPP_WARN(this->get_logger(), "[Publish] Camera %zu has invalid/empty frame", i);
            }
        }
        ...
        if (concatenated.empty()) {
            RCLCPP_WARN(this->get_logger(), "[Publish] Failed to concatenate images, skipping publish");
            continue;
        }
        ...
        // Track and publish FPS based on publish loop
        auto [inst_fps, avg_fps] = coordinator_fps_counter_.tick();
        auto [pub_inst_fps, pub_avg_fps] = publish_fps_counter_.tick();
        auto fps_msg = std_msgs::msg::Float32();
        fps_msg.data = static_cast<float>(pub_avg_fps);
        fps_pub_->publish(fps_msg);

        if (++loop_count % 30 == 0) {
            RCLCPP_INFO(this->get_logger(),
                "[Publish] Loop FPS: %.1f (avg: %.1f)", inst_fps, avg_fps);
            RCLCPP_INFO(this->get_logger(),
                "[Publish] Publish FPS: %.1f (avg: %.1f)", pub_inst_fps, pub_avg_fps);
        }
    }

    RCLCPP_INFO(this->get_logger(), "[Publish] Stopped");
}

void FastStreamNodeCassini::cameraThreadFunc(size_t camera_index)
{
    ...
    // CassiniConversion cassini_converter(cassini_map_file_);
    ...
    while (!stop_flag_ && rclcpp::ok()) {
        ...
        try {
            ...
            if (response.image_data_uint8.size() == response.width * response.height * 3) {
                // RGB image 
                img = cv::Mat(response.height, response.width, CV_8UC3, 
            ...
            // Convert to Cassini projection
            cv::Mat cassini_img = img.clone();
            ...
            // ===== STEP 4: Update frame buffer =====
            {
                ...
                latest_frames_[camera_index].image = cassini_img;
...
cv::Mat FastStreamNodeCassini::concatenateImages(const std::array<FrameInfo, NUM_CAMERAS>& frames)
{
...
sensor_msgs::msg::CompressedImage::SharedPtr FastStreamNodeCassini::createCompressedMsg(
...
sensor_msgs::msg::Image::SharedPtr FastStreamNodeCassini::createRawMsg(
    const cv::Mat& image, uint64_t timestamp_ns)
{
...
int main(int argc, char** argv)
{
    ...
    try {
        auto node = std::make_shared<FastStreamNodeCassini>();
        rclcpp::spin(node);
    } catch (const std::exception& e) {
...
RCLCPP_COMPONENTS_REGISTER_NODE(ct_uav_omni::FastStreamNodeCassini)