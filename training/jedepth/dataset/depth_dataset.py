...
class StereoDepthDataset(Dataset):
    ...
    def __init__(self, csv_path: str, root: str = ".",
        ...
        self.crop_size = crop_size  # (H, W) or None for full resolution

        self.transform = transform or T.ToTensor()

        logger.info(f"Dataset loaded: {len(self.df)} samples (augment={augment}, crop={crop_size})")
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        # Random crop (same location for left, right, disp)
        if self.crop_size is not None:
            left_img, right_img, disp = self._random_crop(left_img, right_img, disp)
    ...
    def _random_crop(self, left: np.ndarray, right: np.ndarray,
                     disp: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        """Random crop stereo pair + disparity to self.crop_size (H, W).
        Same crop location for all three to maintain stereo consistency.
        Only crops vertically — horizontal crop is the same for left/right
        to preserve stereo epipolar alignment.
        """
        crop_h, crop_w = self.crop_size
        img_h, img_w = left.shape[:2]

        # If image is smaller than crop, no crop
        ...
        disp = disp[y:y+crop_h, x:x+crop_w]
...
def create_dataloaders(
    ...
    Args:
        ...
        crop_size: (H, W) tuple for random crop. None for full resolution.
    ...
    if val_csv is not None:
        # Use pre-split CSVs
        train_dataset = StereoDepthDataset(train_csv, root, augment=augment_train, crop_size=crop_size)
        val_dataset = StereoDepthDataset(val_csv, root, augment=False, crop_size=crop_size)
    else:
        ...
        train_dataset = StereoDepthDataset(train_csv, root, augment=augment_train, crop_size=crop_size)
        val_dataset = StereoDepthDataset(train_csv, root, augment=False, crop_size=crop_size)
